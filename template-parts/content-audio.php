...
<article id="post-<?php the_ID(); ?>" <?php post_class(); ?> itemscope="itemscope" itemtype="https://schema.org/BlogPosting" itemprop="blogPost">
	<header class="entry-header">
		...
		$get_audios = get_media_embedded_in_content(apply_filters('the_content', get_the_content()), array('audio', 'iframe'));
		if (is_array($get_audios) && ! empty($get_audios) && ! post_password_required() && ! is_attachment()): ?>
		<div class="entry-thumb">
			<?php echo $get_audios[0]; ?>
		</div><!-- .entry-thumb -->
		<?php 
		endif;
		if (is_singular()):
